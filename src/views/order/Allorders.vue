       ...
       <el-col :span="4" style="margin-bottom:15px">
        ...
        <el-table :data="tableData"  border style="width: 100%" >      
          ...
          <el-table-column  label="订单详情" width="420">
            <div class="cell">
                <!-- <template slot-scope="scope"> -->
                <div class="pros"  v-for="(itemGoods,index) in listOrderdetail" :key="index">
          ...
          <el-table-column label="支付状态" prop="ordertype" width="100">
            <template slot-scope="scope">
                <div v-if="scope.row.ordertype==1">待支付</div>
                <div v-if="scope.row.ordertype==2">待发货</div>
                <div v-if="scope.row.ordertype==3">已发货</div>
                <div v-if="scope.row.ordertype==4">用户取消</div>
                <div v-if="scope.row.ordertype==5">后台取消</div>
                <div v-if="scope.row.ordertype==6">退款</div>
                <div v-if="scope.row.ordertype==0">已完成</div>
              </template>
          </el-table-column>
...
import { OrderSelect,OrderIdSelect,UpDateOrder } from "@/api/order.js";
export default {
  data() {
    return {
      listOrderdetail:[ ],
